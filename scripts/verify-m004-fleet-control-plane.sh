...
if [[ "${OMNI_M004_LIVE:-0}" == "1" ]]; then
  echo "== M004 live repo/network/db validation =="
  python3 modules/fleet-control-plane/tools/validate_m004.py --live --json
else
  echo "== M004 live repo/network/db validation skipped =="
  echo "Set OMNI_M004_LIVE=1 to probe SRV1/SRV2/SRV3 over SSH and PgBouncer."
fi
