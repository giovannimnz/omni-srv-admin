...
The public alias `embedding-gte-v1` resolves through our `router-ai-atius` / New API gateway to the private TEI upstream `http://10.100.100.4:3115`, which serves `Alibaba-NLP/gte-multilingual-base` through upstream model name `embedding-gte-v1`.
