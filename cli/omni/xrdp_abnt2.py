...
def _watchdog_target(username: str) -> Path:
    return _user_home(username) / "bin" / "setxkbmap-abnt2.sh"
...
def _install_file(src: Path, dst: Path, mode: str, owner: str, group: str, dry_run: bool) -> None:
    _run(["install", "-o", owner, "-g", group, "-m", mode, str(src), str(dst)], dry_run=dry_run)
...
def install_cmd(username: str, yes: bool, dry_run: bool) -> None:
