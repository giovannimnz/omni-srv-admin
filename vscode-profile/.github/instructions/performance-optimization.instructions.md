...
Performance não é só um termo da moda—é a diferença entre um produto que as pessoas amam e um que elas abandonam. Já vi na prática como um app lento frustra usuários, aumenta custos de nuvem e até faz perder clientes. Este guia é uma coleção viva das práticas de performance mais eficazes e reais que já usei e revisei, cobrindo frontend, backend e banco de dados, além de tópicos avançados. Use como referência, checklist e fonte de inspiração para construir software rápido, eficiente e escalável.

---



- **Meça antes, otimize depois:** Sempre faça perfilamento e medições antes de otimizar. Use benchmarks, profilers e ferramentas de monitoramento para identificar gargalos reais. Chutar é o inimigo da performance.
  - *Dica Pro:* Use ferramentas como Chrome DevTools, Lighthouse, New Relic, Datadog, Py-Spy ou os profilers nativos da linguagem.
- **Otimize para o caso comum:** Foque em otimizar os caminhos de código mais executados. Não perca tempo com casos raros, a menos que sejam críticos.
- **Evite otimização prematura:** Escreva código claro e manutenível primeiro; só otimize quando necessário. Otimização prematura pode dificultar a manutenção.
- **Minimize o uso de recursos:** Use memória, CPU, rede e disco de forma eficiente. Sempre pergunte: "Dá para fazer com menos?"
- **Prefira simplicidade:** Algoritmos e estruturas simples geralmente são mais rápidos e fáceis de otimizar. Não complique demais.
- **Documente suposições de performance:** Comente claramente qualquer código crítico para performance ou com otimizações não óbvias. Futuros mantenedores (inclusive você) agradecem.
- **Entenda a plataforma:** Conheça as características de performance da sua linguagem, framework e runtime. O que é rápido em Python pode ser lento em JavaScript, e vice-versa.
- **Automatize testes de performance:** Integre testes e benchmarks de performance no seu CI/CD. Pegue regressões cedo.
- **Defina orçamentos de performance:** Estabeleça limites aceitáveis para tempo de carregamento, uso de memória, latência de API etc. Faça cumprir com checagens automáticas.
