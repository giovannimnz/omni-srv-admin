...
must_haves:
  truths:
    ...
    - "The governed current broad RustDesk lane exits zero after deselecting exactly the closed nine legacy Gate-B nodeids; inline JUnit parsing proves zero failures/errors and a complete skipped-case list containing exactly one pytest.xfail at the current Phase 53 validate_phase53.py owner nodeid, while the terminal summary proves no other deselections."
    - "The observed current-lane baseline is 902 passed, 9 deselected, 1 xfailed at rc 0; the frozen Phase 52 two-xfail current-lane helper does not govern this Phase 53 source because its removed deploy-transaction xfail is now legitimately implemented."
...
| T53C-LANES | Tampering | broad test interpretation | high | mitigate | Current lane explicitly deselects only the closed nine nodeids, parses the complete JUnit skip set inline for the one current Phase 53 xfail and rejects any other outcome/deselection; temporary legacy JUnit plus read-only lane_legacy proves the exact expected refusal set without applying stale Phase 52 xfail policy or rewriting Gate A/evidence. |
</threat_model>
...
3. The current broad lane exits zero with exactly nine explicit deselections, zero failures/errors and exactly the one current validate_phase53.py pytest.xfail; every regular skip, second/new xfail or extra deselection blocks.
