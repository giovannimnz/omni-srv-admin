...
- **D-17:** The existing `edge-forwarder-operation-plan.json` is stale, unapproved and bound to an older source. It is forbidden as an execution input or approval hash. The 05D2C 33-path seal is an immutable historical ancestor but became non-authorizing when the authority-producer gap was reproduced. Plan 53-05D2D creates the superseding current 34-path source—whose ancestry includes 05D2C—and 53-05D2H recoverably quarantines stale canonical 05F outputs before authority. Only then may 53-05E generate a new OperationPlan from current read-only topology/prestate; no prior hash, typed confirmation or approval may be reused.
