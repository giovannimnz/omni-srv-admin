...
**Last updated:** 2026-06-13 after M004 agent executor and monitoring extension
...
**Current focus:** M004 Omni Fleet Control Plane is live for its intended base: `~/GitHub/omni-srv-admin` exists on SRV1/SRV2/SRV3, SRV-1 owns central PostgreSQL database `DbOmniFleet`, and SRV-2/SRV-3 query it through PgBouncer on the private endpoint. `DbOmniFleet` is also the canonical `omni-srv-admin` DB for ops scopes, configs, runtime parameters, slash-command registry, agent execution queue and fleet telemetry. M005 K3s HA Cluster + Portainer is queued in its separate branch.
...
| M004 | Omni Fleet Control Plane (Phase 12, branch `codex/omni-fleet-control-plane-m004`) | Live implemented; repos, central DB, DB-backed ops/config/slash registry, agent executor and monitoring contract validated |
...
| Module | `modules/fleet-control-plane/` criado com config exemplo, migrations `0001..0003`, PgBouncer guard e unit/installer do agent | ✅ |
| CLI | `validate-inventory`, `install server/node`, `heartbeat`, `programs`, `update-plan`, `queue-update`, `agent heartbeat/once/loop`, `monitor hosts`, `audit`, `status --all` | ✅ |
...
| PostgreSQL | SRV-1 database `DbOmniFleet` exists with initial schema and seeded `TbHosts`/`TbNodes`/`TbPrograms` | ✅ |
| DB-backed ops/config | `0002_ops_config_slash_commands.sql` defines `TbOpsScopes`, `TbConfigItems`, `TbSlashCommands` and CLI-Anything bindings | ✅ |
| Agent executor | `0003_agent_executor_monitoring.sql` adds `TbFleetCommands`, execution leases/idempotency on `TbUpdatePlans`, local allowlist execution and audit output | ✅ |
| Fleet monitoring | `TbNodeTelemetry` and `TbNodeResourcePolicies` store load, CPU, memory, disk, I/O, pressure and service health for cross-server visibility; SRV-1 telemetry written live | ✅ |
| PgBouncer | SRV-1 PgBouncer is active on `127.0.0.1:6432` and `10.1.1.1:6432`; SRV-1/SRV-2/SRV-3 query `DbOmniFleet` via PgBouncer and direct node access to `8745` is blocked | ✅ |
...
| Remaining rollout | Enable `omni-fleet-agent.service` on SRV-2/SRV-3 after the branch is distributed to their canonical `~/GitHub/omni-srv-admin` path | Open |

## M004 Phase Breakdown

| Milestone | Phase | Descrição | Status |
|---|---:|---|---|
| M004 | 12 | Fleet Control Plane Foundation | Live implemented; repos, central DB, DB-backed ops/config/slash registry and agent executor/monitoring contract validated |
