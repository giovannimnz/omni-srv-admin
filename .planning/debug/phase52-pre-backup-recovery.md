...
- verification: V13 snapshot coverage returned 18 passes. V14/V15 add supervisor/cleanup coverage for child exit 0/2, validation failure, cleanup failure, TERM, unexpected exit, and descendant teardown. The complete governed regression now returns 512 passes with exit 0 and `structural_ok=true`. Rejected hashes `8fb6f149...` and `f928f0f6...` are superseded; a new seal and two new hash-bound PASS reviews remain required.
...
- residual_risk: transaction `20260722T163940Z-cf840f4c` remains `ROLLBACK_BLOCKED_RETRY_REQUIRED` from the pre-V12 recovery path; transaction `20260722T181735Z-379852e2` is safely terminal as `PRE_BACKUP_NO_MUTATION_TERMINAL`. Both have zero Vault writes. The V13 bridge is locally verified but not yet exercised by a newly sealed live transaction.
