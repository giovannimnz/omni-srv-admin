...
- root_cause: the production wrapper runs the Vault CLI inside `hashicorp-vault-atius`, but `LocalVaultBackend.create_backups` passed a host-only `/var/backups/.../raft.snapshot` path to `vault operator raft snapshot save`. The CLI resolved that path inside the container and failed before creating the first backup artifact. The earlier recovery bug then obscured this by sending PRE_BACKUP through a restore path; V9-V12 fixed state classification and sanitization but not the namespace mismatch.
- fix: V9-V12 retain the exact PRE_BACKUP no-mutation terminal and finite blocker/output boundaries. V13 saves the Raft snapshot to an unpredictable temp path inside the Vault container, copies it with `/usr/bin/podman cp` into a private staging directory under the transaction, validates the staged inode through one `O_NOFOLLOW` fd, removes the container temp before publication, and atomically publishes with fail-if-exists `os.link`; all setup, subprocess, cleanup, validation, and publish failures collapse to `raft-snapshot-failed` without overwrite.
- verification: V13 snapshot coverage returned 18 passes; V14/V15 supervisor coverage brought the clean regression to 512. V16 adds 7 overlay-binary routing cases (21 focused passes including the snapshot bridge). A clean complete rerun, new seal, and two new hash-bound PASS reviews are required before resuming the zero-write transaction and starting another live transaction.
...
- residual_risk: transaction `20260722T163940Z-cf840f4c` remains `ROLLBACK_BLOCKED_RETRY_REQUIRED`; `20260722T181735Z-379852e2` is `PRE_BACKUP_NO_MUTATION_TERMINAL`; `20260722T190633Z-0943450c` remains PRE_BACKUP pending safe terminalization after the next seal. All three have zero Vault writes. V16 overlay-binary routing is locally/capability verified but not yet exercised by a newly sealed live transaction.
