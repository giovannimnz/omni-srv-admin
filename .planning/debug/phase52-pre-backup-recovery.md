...
- fix: V9-V12 retain the exact PRE_BACKUP no-mutation terminal and finite blocker/output boundaries. V13-V16 bridge and validate the snapshot across the container namespace, supervise cleanup descendants, and route the validated container Vault binary. V17 supplies explicit loopback API/cluster addresses, creates the private Raft directory, validates `vault.db`, calls the correct force-restore endpoint, and proves the sealed restart through initialized/sealed health plus changed durable Raft digest.
- verification: the V17 focused tests passed, the retained production snapshot completed the exact isolated restore proof with no host/public listener, and the governed complete regression returned 562 passes. A new seal and two new hash-bound PASS reviews are still required before terminalizing the current zero-write transaction and starting another live transaction.
...
- residual_risk: transaction `20260722T163940Z-cf840f4c` remains `ROLLBACK_BLOCKED_RETRY_REQUIRED`; `20260722T181735Z-379852e2` and `20260722T190633Z-0943450c` are `PRE_BACKUP_NO_MUTATION_TERMINAL`; `20260722T191932Z-fe82bbca` remains PRE_BACKUP pending safe terminalization after the next seal. All four have zero Vault writes. V17 is proven against the retained live snapshot but has not yet passed the full sealed transaction sequence.
