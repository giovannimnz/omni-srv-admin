...
> Versionado no git junto com `sync.yaml`. CLI regenera com `fork-sync manuals regen hermes-agent`.
...
| # | Nome | URL | Role |
|---|------|-----|------|
| 1 | `hermes-agent` | https://github.com/NousResearch/hermes-agent | primary |
...
```bash
fork-sync manuals regenerate hermes-agent
```
